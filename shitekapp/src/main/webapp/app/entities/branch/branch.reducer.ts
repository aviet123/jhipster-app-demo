...
export const ACTION_TYPES = {
  ...
  PARTIAL_UPDATE_BRANCH: 'branch/PARTIAL_UPDATE_BRANCH',
...
export default (state: BranchState = initialState, action): BranchState => {
  switch (action.type) {
    ...
    case REQUEST(ACTION_TYPES.PARTIAL_UPDATE_BRANCH):
      return {
        ...state,
        errorMessage: null,
        updateSuccess: false,
    ...
    case FAILURE(ACTION_TYPES.PARTIAL_UPDATE_BRANCH):
    ...
    case SUCCESS(ACTION_TYPES.PARTIAL_UPDATE_BRANCH):
      return {
        ...state,
        updating: false,
        updateSuccess: true,
...
export const partialUpdate: ICrudPutAction<IBranch> = entity => async dispatch => {
  const result = await dispatch({
    type: ACTION_TYPES.PARTIAL_UPDATE_BRANCH,
